...
const char WALL_FILEPATH[] = "wall1.png";
...
void initialise()
{
    ...
    for (int i = 0; i < PLATFORM_COUNT; i++)
    {
        ...
        state.platforms[i].set_position(glm::vec3(i + 1.5f, -3.0f, 0.0f));
    ...
    GLuint wall_texture_id = load_texture(WALL_FILEPATH);
    for (int i = 0; i < WALL_COUNT; i++)
    {
        state.wall[i].texture_id = wall_texture_id;
    }

    state.wall[0].set_position(glm::vec3(-0.5f, 0.0f, 0.0f));
    state.wall[1].set_position(glm::vec3(-1.5f, 0.0f, 0.0f));
    state.wall[2].set_position(glm::vec3(4.0f, -0.5f, 0.0f));
    state.wall[3].set_position(glm::vec3(-2.5f, 0.0f, 0.0f));
    state.wall[4].set_position(glm::vec3(-3.5f, 0.0f, 0.0f));
    state.wall[5].set_position(glm::vec3(-4.5f, 0.0f, 0.0f));
    for (int i = 0; i < WALL_COUNT; i++)
    {
        state.wall[i].update(0.0f, NULL, 0.0f);
        state.wall[i].type = ITEM;
    }
    ...
    state.player->set_acceleration(glm::vec3(0.0f, -0.75f, 0.0f));
    state.player->texture_id = load_texture(SPRITESHEET_FILEPATH);
    state.player->type = PLAYER;

    // initialize player collision first 
...
void process_input()
{
    ...
    if (state.player->collision != PLAYER) {
        state.player->set_movement(glm::vec3(0.0f));
    ...
    {
        state.player->acceleration.x -= 0.15f;
...
void update()
{
    ...
    while (delta_time >= FIXED_TIMESTEP) {
        ...
        state.player->update(FIXED_TIMESTEP, state.wall, WALL_COUNT);
...
void render()
{
    ...
    for (int i = 0; i < WALL_COUNT; i++) state.wall[i].render(&program);
    for (int i = 0; i < PLATFORM_COUNT; i++) state.platforms[i].render(&program);
    state.player->render(&program);

    if (state.player->collision == PLATFORM) {
        // win
        DrawText(&program, the_texture_id, "Mission Successful", 1.0f, -0.6f, glm::vec3(-3.5, 0.0, 0));
    }
    else if (state.player->collision == ITEM) {
        // game over
        DrawText(&program, the_texture_id, "Mission Failed", 1.0f, -0.6f, glm::vec3(-3.5, 0.0, 0));
    }
